...
MiscObject Property Provisions Auto
	...
	if (player.IsInLocation(Sovangarde) == false)
		if (mcmOptions._lastBed == true)
			...
			elseif player.GetDistance(PlayerRespawnMarker) < 300
				...
				RespawnToLocation(altarTempleOfKynareth)
			ElseIf PO3_SKSEFunctions.IsSurvivalModeActive() && ! player.GetItemCount(GetProvisions())
				RespawnToNearestTemple()
            Else
                RespawnToLocation(PlayerRespawnMarker)
                If PO3_SKSEFunctions.IsSurvivalModeActive()
                    player.RemoveItem(GetProvisions())
                EndIf
			endif
...
Function RespawnToNearestTemple()
...
MiscObject Function GetProvisions()
    If ! Provisions
        Provisions = Game.GetFormFromFile(0x800, "LoreRim Respawn Provision.esp") as MiscObject
    EndIf
    Return Provisions
EndFunction